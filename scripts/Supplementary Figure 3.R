...
p2 <- plot_activity(df, repair_Uracil_45, seurat_clusters) + 
        theme(legend.position = "none") + 
...
p3 <- plot_activity(df, RNASEH2C, seurat_clusters) + 
        ...
        labs(x = "mRNA expression")
p4 <- plot_activity(df, repair_riboG_44, seurat_clusters) + 
        theme(legend.position = "none") + 
...
df %>% mutate(UNG_expression = ifelse(UNG == 0, 
              ...
              RNASEH2C_expression = ifelse(RNASEH2C == 0,
                                           "0", "> 0"),
              Uracil_repair = ifelse(repair_Uracil_45 == 0, 
                                     "0", "> 0"),
              Ribonucleotide_repair = ifelse(repair_riboG_44 == 0,
                                             "0", "> 0")) %>%
...
count_df <- rownames_to_column(FetchData(barnyard_seurat, 
                                         vars = c("Uracil-45", "riboG-44", "seurat_clusters"),
                                         slot = 'counts'),
                               "cell_id")
...
count_df %>% ggplot(aes(x = `repair_Uracil-45`, 
                        y = `repair_riboG-44`, 
